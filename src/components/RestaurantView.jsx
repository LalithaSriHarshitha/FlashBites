...
import { 
  ...
  DollarSign, PackageCheck, AlertCircle, Edit, PlusCircle, ToggleLeft, ToggleRight, Plus, Upload
} from 'lucide-react';
...
export default function RestaurantView({
  ...
  const [stockItems, setStockItems] = useState([
    { id: 'm1', name: 'Special Masala Dosa', price: 120, isAvailable: true, category: 'Dosa', image: 'https://images.unsplash.com/photo-1589301760014-d929f3979dbc?w=400&auto=format&fit=crop&q=80' },
    { id: 'm2', name: 'Ghee Roast Paper Dosa', price: 160, isAvailable: true, category: 'Dosa', image: 'https://images.unsplash.com/photo-1668236543090-82eba5ee5976?w=400&auto=format&fit=crop&q=80' },
    { id: 'm3', name: 'Steam Idli & Medu Vada Combo', price: 90, isAvailable: true, category: 'Tiffin', image: 'https://images.unsplash.com/photo-1589301760014-d929f3979dbc?w=400&auto=format&fit=crop&q=80' },
    { id: 'm4', name: 'Filter Coffee (Degree Coffee)', price: 30, isAvailable: true, category: 'Drinks', image: 'https://images.unsplash.com/photo-1514432324607-a09d9b4aefdd?w=400&auto=format&fit=crop&q=80' }
  ]);
  ...
  const activeCookingOrders = orders.filter((o) => ['PLACED', 'CONFIRMED', 'PREPARING'].includes(o.status));
  ...
  const handleAddDishSubmit = (e) => {
    ...
    setStockItems((prev) => [dishToAdd, ...prev]);
    ...
    <div className="space-y-6 pb-16">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-200 shadow-xl">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">Sri Udipi Grand • Order Fulfillment Console</p>
          </div>
        </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 pb-2">
        <div className="flex space-x-2">
          ...
          >
            👨‍🍳 Live Kitchen Queue ({orders.filter(o => o.status !== 'CANCELLED' && o.status !== 'DELIVERED').length})
          </button>
        ...
        <div className="space-y-4">
          {orders.length === 0 ? (
            <div className="bg-white rounded-3xl p-12 text-center text-slate-400 border border-slate-200 shadow-sm">
              ...
              <p className="text-sm font-semibold">Kitchen queue empty right now</p>
            </div>
          ) : (
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 pb-3 mb-3">
                      <div>
                        ...
                        <p className="text-xs text-slate-500 mt-0.5">
                          Customer: <strong className="text-slate-800">{ord.customer_name || 'Alex Rivera'}</strong> ({ord.customer_phone || '+91 98765 43210'})
                        </p>
                      </div>
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
                        <div className="flex items-center space-x-2">
                          ...
                          {ord.status === 'PREPARING' && (
                            <button
                              onClick={() => onUpdateStatus(ord.id, 'READY_FOR_PICKUP')}
                              className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-extrabold transition-all flex items-center space-x-1 shadow-sm"
                            >
                              <Bike className="w-4 h-4" />
                              <span>Mark Ready & Auto-Assign Driver</span>
                            </button>
                          )}
                          ...
                          {['READY_FOR_PICKUP', 'PICKED_UP', 'OUT_FOR_DELIVERY'].includes(ord.status) && (
                            <div className="text-xs text-emerald-700 flex items-center space-x-1 font-bold bg-emerald-50 px-3 py-1.5 rounded-xl border border-emerald-200">
                              ...
                              <span>Handed over to Partner: {ord.driver_name || 'Marcus Chen'}</span>
                            </div>
                          )}
                        </div>
      ...
      {activeTab === 'MENU_STOCK' && (
        <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-xl space-y-4">
          <div className="flex justify-between items-center border-b border-slate-100 pb-3">
            <div>
              <h3 className="font-extrabold text-slate-900 text-base">Menu Item Stock Controller</h3>
          ...
          <div className="space-y-3">
            {stockItems.map((item) => (
              <div key={item.id} className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-200">
                ...
                >
                  {item.isAvailable ? <ToggleRight className="w-5 h-5 text-emerald-600" /> : <ToggleLeft className="w-5 h-5 text-rose-600" />}
                  <span>{item.isAvailable ? 'In Stock (Active)' : 'Out of Stock'}</span>
                </button>
              </div>
            ))}
          </div>
      ...
      {/* UPLOAD DISH MODAL FORM - NO CATEGORY DROPDOWN */}
      {isAddDishOpen && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white max-w-md w-full rounded-3xl p-6 border border-slate-200 shadow-2xl relative space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="font-extrabold text-slate-900 text-base">Upload New Dish to Menu</h3>
